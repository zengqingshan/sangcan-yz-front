  ...
  <div class="contrion">
    ...
    >
      <span
        ...
        @click="playvedios(data)"
      >
        <span class="child">
          <span>
            <i
              :class="{ 'el-icon-folder': data.type == 'org' }"
              style="color: #5d5d5d"
            />
            <span
              v-if="data.type == 'device'"
              style="color: #46a0fc; margin-right: 5px"
            >
              <svg width="1em" height="1em" viewBox="0 0 16 16">
                ...
                >
                  <path
                    d="M8 1.5999999999999996A5.4 5.4 0 1 0 8 12.4A5.4 5.4 0 1 0 8 1.5999999999999996Z"
                  />
                  ...
                  <path
                    ...
                    d="M4.75724981 14.7662345L11.3730759 14.7662345"
                  />
                </g>
              </svg>
            </span>
          ...
          <el-popover placement="right" width="130" trigger="click">
            <el-button
              v-if="data.type == 'device'"
              @click="cancelDeviceStar(data)"
              ><i class="el-icon-star-off" />取消关注</el-button
            >
            <i
              v-if="data.type == 'device'"
              slot="reference"
    ...
    <el-button v-else type="primary" @click="loadingmoreOrgStarList"
      >加载更多</el-button
    >
  </div>
</template>
...
<script>
var num = 0;
import { listPageStarOrg } from "@/api/system/org";
import { listPageStarDevice, deviceStarDelete } from "@/api/system/device";
...
export default {
  ...
  data() {
    return {
      ...
      currentall: 1,
      concernlist: [
        { type: "org", name: "全部关注设备", total: 0, children: [] },
      ], // 关注列表
  ...
  methods: {
    ...
    getconcernlist(firstLoad) {
      listPageStarDevice({ current: this.current, pageSize: 10 }).then(
        (res) => {
          ...
          if (firstLoad) {
            ...
            if (this.current > 1) {
              // 首页已经加载的时候追加了
              const childrentList =
                this.concernlist[0].children.concat(recList);
    ...
    getOrgStarlist(firstLoad) {
      ...
      listPageStarOrg({ current: this.currentall, pageSize: 10 }).then(
        (res) => {
