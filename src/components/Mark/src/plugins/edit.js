    ...
    instance.on('DOUBLE-CLICK', (e) => {
        ...
        if (curEditingMarkItem) {
            ...
            if (opt.dbClickRemovePoint && inPointIndex !== -1) {
                canActive = false
                if (curEditingMarkItem.getPointLength() > opt.minPointLength) {
                ...
                } else if(opt.maxPointLength && curEditingMarkItem.getPointLength() > opt.maxPointLength){
                    canActive = false
                    instance.emit('NOT-MORE-END-POINTS', curEditingMarkItem)
