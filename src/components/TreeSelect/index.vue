...
<template>
    <div>
        ...
        <el-popover placement="bottom-start" trigger="manual" v-model="isShowSelect" @hide="popoverHide">
            <el-tree class="common-tree" :style="style" ref="tree" :data="data" :props="defaultProps" :show-checkbox="multiple" :node-key="nodeKey" :check-strictly="checkStrictly" default-expand-all :expand-on-click-node="false" :check-on-click-node="multiple" :highlight-current="true" @node-click="handleNodeClick" @check-change="handleCheckChange" @check='onCheck'></el-tree>
            <el-select :style="selectStyle" slot="reference" ref="select" :size="size" v-model="selectedData" :multiple="multiple" :clearable="clearable" :collapse-tags="collapseTags" @click.native="isShowSelect = !isShowSelect" @remove-tag="removeSelectedNodes" @clear="removeSelectedNode" @change="changeSelectedNodes" class="tree-select">
...
export default {
    ...
    methods: {
        ...
        clearSelectedNodes() {
            ...
            this.$emit("check", this.selectedData);
        ...
        popoverHide() {
            if (this.multiple) {
                this.checkedData = this.$refs.tree.getCheckedNodes(true); // 所有被选中的节点所组成的数组数据
            } else {
                this.checkedData = this.$refs.tree.getCurrentNode();
            }
            this.$emit("popoverHide", this.checkedData);
        },
        ...
        // 多选，节点勾选状态发生变化时的回调
        handleCheckChange() {
            const checkedKeys = this.$refs.tree.getCheckedKeys(true); // 所有被选中的节点的 key 所组成的数组数据
