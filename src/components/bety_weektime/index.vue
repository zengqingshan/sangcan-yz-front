  ...
  <div class="weektime">
    <div class="weektime-main">
      <div class="weektime-hd">
        ...
        <div class="weektime-hd-con">
          ...
          <div class="weektime-hd-con-bottom">
            <span class="weektime-date-cell" v-for="hour in 24" :key="hour">{{
              hour - 1
            }}</span>
          </div>
        </div>
      ...
      <div class="weektime-bd">
        <div class="week-body">
          <div v-for="week in weekDays" :key="week" class="week-item">
            {{ week }}
          </div>
        </div>
        <div
...
export default {
  ...
  methods: {
    ...
    tiptxt(index) {
      ...
      return `${this.weekDays[weekIndex]} ${this.timeTextList[timeIndex]}~${
        this.timeTextList[timeIndex + 1]
      }`;
    },
    /**
     * 初始化显示的时间数组
      ...
      arrIndex.forEach((value, index) => {
        if (value === "1") {
          ...
          if (index === timeLength - 1)
            timeText += "-" + this.timeTextList[index + 1] + "、";
        } else {
          if (isSelect) {
            timeText += "-" + this.timeTextList[index] + "、";
            isSelect = false;
    ...
    initdata(data) {
      ...
      data.forEach((value, index, array) => {
        ...
        if (value) {
          ...
          b.forEach((e) => {
            let c = e.split("-");
            let strObj = { onef: "", ones: "", twof: "", twos: "" };
            c.forEach((d, i) => {
              d.replace(/(\d+):(\d+)/);
              strObj[i ? "twof" : "onef"] = RegExp.$1 * 1;
              strObj[i ? "twos" : "ones"] = RegExp.$2 * 1;
            });
            let startindex = 0;
            let Executionlength = 0;
            // 起始索引
            startindex = strObj.ones
              ? (startindex = strObj.onef * 2 + 1)
              : (startindex = strObj.onef * 2);
            // 执行长度
            // Executionlength = !strObj.ones && !strObj.twos ? (strObj.twof - strObj.onef) * 2 : strObj.ones && !strObj.twos? (Executionlength = (strObj.twof - strObj.onef) * 2 - 1) : (Executionlength = (strObj.twof - strObj.onef) * 2 + 1);
            Executionlength =
              !strObj.ones && !strObj.twos
                ? (strObj.twof - strObj.onef) * 2
                : strObj.ones && strObj.twos
                ? (strObj.twof - strObj.onef) * 2
                : strObj.ones && !strObj.twos
                ? (strObj.twof - strObj.onef) * 2 - 1
                : (strObj.twof - strObj.onef) * 2 + 1;
            a = this.replaceAt(a, startindex, "".padStart(Executionlength, 1));
          });
        }
        j += a;
      });
